...
object ADALMiddleware extends LazyLogger {
  ...
  * retryPolicy is expressed in terms of IO.
   ...
   * @return new Middleware and a "stop" effect.
    */
  def apply[F[_]](info: ConnectionInfo, retryPolicy: IO[TokenInfo] => IO[TokenInfo],
    updateAfter: TokenInfo => FiniteDuration = AuthManager.defaultCalc)
    (implicit sch: Scheduler, timer: Timer[F], F: Async[F],
      ec: ExecutionContext, IOtoF: LiftIO[F]): Middleware[F] =
    (c: Client[F]) => {
      ...
      val xf: HttpRequest[F] => F[HttpRequest[F]] =
        request => IOtoF.liftIO(token.map { ti =>
