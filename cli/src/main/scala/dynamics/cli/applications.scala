...
class ApplicationActions(val context: DynamicsContext) extends {
  ...
  val list = Action { config =>
    println("AppModules: See https://docs.microsoft.com/en-us/dynamics365/customer-engagement/web-api/appmodule?view=dynamics-ce-odata-9")
    val cols = lit(
      // description
      "14" -> lit(width=40)
    )
    val topts = new TableOptions(
      border= Table.getBorderCharacters(config.common.tableFormat),
      columns=cols)
