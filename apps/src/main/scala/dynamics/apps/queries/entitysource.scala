...
class DynamicsEntitySource(val dynclient: DynamicsClient, verbosity: Int = 0) {
  ...
  def fetchItemsFromFetchXMLQuery[A <: js.Object](qry: FetchXMLQuery, context: js.Object): Either[String, Stream[IO, A]] = {
  ...
  def fetchItems[A <: js.Object](query: ODataQuery|FetchXMLQuery, context: js.Object): Either[String, Stream[IO, A]] = {
    (query.asInstanceOf[js.Any]) match {
      case q if query.merge[js.Object].hasOwnProperty("odata") &&
          (q.asDyn.odata != null) =>
        fetchItemsFromODataQuery(q.asInstanceOf[ODataQuery], context)

      case q if query.merge[js.Object].hasOwnProperty("fetchXML") &&
