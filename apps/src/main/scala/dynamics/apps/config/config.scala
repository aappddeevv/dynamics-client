...
import dynamics.common.{Utils, IOUtils}
...
class Config(val dynclient: DynamicsClient, verbosity: Int = 0)(implicit ec: ExecutionContext) {
  ...
  def getLocalConfig[A <: js.Object](name: String): IO[A] =
    IO.pure(IOUtils.slurpAsJson[A](name))
  ...
  /**
    ...
    */
  def getRemoteConfiguration(name: String,
                             entitySetName: String,
                             nameAttribute: String,
                             contentAttribute: String): IO[Option[String]] = {
    val q = QuerySpec(
      select = Seq(nameAttribute),
      filter = Some(s"""$nameAttribute eq '$name'""")
    )
    dynclient
      .getList[ValueArrayResponse[js.Object]](q.url(entitySetName))
      .map { seq =>
